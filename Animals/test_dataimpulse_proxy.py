"""
Full Animals scrape test using DataImpulse residential proxies geo-targeted to Kuwait.

What this does:
  1.  Fetches ALL Animals subcategories from q84sale.com/ar/animals
  2.  For each subcategory checks for child categories
  3.  Paginates through listing pages — NO date filter (scrapes everything)
  4.  Optionally fetches individual listing detail pages
  5.  Saves the full result to a local JSON file (no S3 needed)
  6.  Prints a final report including the TOTAL NUMBER OF HTTP REQUESTS made

Proxy:  DataImpulse residential  (gw.dataimpulse.com:823)
Geo:    Kuwait  (KW) — appended as __cr.kw to the username

Usage:
    python test_dataimpulse_proxy.py
    python test_dataimpulse_proxy.py --max-pages 0 --max-details 5
    python test_dataimpulse_proxy.py --max-pages 3 --output my_output.json

CLI arguments:
    --max-pages    Max listing pages per subcategory  (0 = unlimited, default: 5)
    --max-details  Max detail-page fetches per subcat (0 = skip,      default: 0)
    --output       Output JSON file path              (default: animals_dataimpulse_result.json)

Environment variables (override defaults):
    DATAIMPULSE_USER   proxy username  (default: 2ced8727319e5746c47f__cr.kw)
    DATAIMPULSE_PASS   proxy password  (default: ee22dc54c002e494)
    DATAIMPULSE_HOST   proxy host      (default: gw.dataimpulse.com)
    DATAIMPULSE_PORT   proxy port      (default: 823)
"""

import argparse
import asyncio
import json
import logging
import os
import random
import sys
import time
from datetime import datetime
from pathlib import Path

# Allow importing scraper_utils and json_scraper from parent / same directory
sys.path.insert(0, str(Path(__file__).parent.parent))

from curl_cffi import requests as curl_requests

logging.basicConfig(
    level=logging.INFO,
    format="%(asctime)s %(levelname)-8s %(message)s",
    datefmt="%H:%M:%S",
)
logger = logging.getLogger(__name__)


# ─── DataImpulse proxy session ────────────────────────────────────────────────

_IMPERSONATION_PROFILES = [
    "chrome124",
    "chrome120",
    "chrome131",
    "safari18_0",
    "chrome116",
]


class _CachedResponse:
    """Minimal response-like object served from the URL cache (costs 0 requests)."""

    status_code = 200

    def __init__(self, text: str):
        self.text = text
        self.content = text.encode("utf-8")

    def raise_for_status(self):
        pass


class DataImpulseSession:
    """
    curl_cffi-based session that routes every request through DataImpulse
    residential proxies geo-targeted to Kuwait.

    Tracks:
        request_count  — real HTTP requests sent through the proxy
        cache_hits     — duplicate URL fetches served from memory (free, no request)

    Auto-retries on HTTP 403 by rotating TLS impersonation profiles.
    Caches responses so the same URL is never fetched twice (catchilds and
    listings page-1 share the same URL — the second call is always free).
    """

    def __init__(
        self,
        user: str,
        password: str,
        host: str = "gw.dataimpulse.com",
        port: int = 823,
    ):
        proxy_url = f"http://{user}:{password}@{host}:{port}"
        self._proxies = {"http": proxy_url, "https": proxy_url}
        self._profile_index = 0
        self._session = curl_requests.Session(
            impersonate=_IMPERSONATION_PROFILES[0],
            proxies=self._proxies,
        )
        self._cache: dict = {}  # url -> response text
        self.request_count = 0  # actual proxy requests made
        self.cache_hits = 0     # requests served from in-memory cache

    def _next_profile(self) -> str:
        self._profile_index = (self._profile_index + 1) % len(_IMPERSONATION_PROFILES)
        return _IMPERSONATION_PROFILES[self._profile_index]

    def get(self, url: str, **kwargs):
        # Serve from cache — zero cost
        if url in self._cache:
            self.cache_hits += 1
            logger.debug(f"Cache hit: {url}")
            return _CachedResponse(self._cache[url])

        last_exc = None
        for attempt in range(len(_IMPERSONATION_PROFILES)):
            try:
                if attempt > 0:
                    profile = self._next_profile()
                    logger.warning(
                        f"403 received — retrying with profile '{profile}' "
                        f"(attempt {attempt + 1}/{len(_IMPERSONATION_PROFILES)})"
                    )
                    time.sleep(random.uniform(2.0, 4.0))
                    self._session = curl_requests.Session(
                        impersonate=profile,
                        proxies=self._proxies,
                    )

                response = self._session.get(url, **kwargs)

                if response.status_code == 403:
                    last_exc = Exception(f"HTTP 403 for {url}")
                    continue

                # Success — count it and cache it
                self.request_count += 1
                self._cache[url] = response.text
                return response

            except Exception as exc:
                if "403" in str(exc):
                    last_exc = exc
                    continue
                raise

        raise last_exc or Exception(
            f"All {len(_IMPERSONATION_PROFILES)} retry attempts failed for {url}"
        )

    def close(self):
        if self._session:
            self._session.close()

    @property
    def headers(self):
        return self._session.headers

    @property
    def proxies(self):
        return self._proxies


# ─── Core scraping logic ──────────────────────────────────────────────────────

async def scrape_all(scraper, max_pages: int, max_details: int) -> list:
    """
    Scrape all Animals subcategories — no date filtering, all listings.

    max_pages  == 0  means paginate until the page returns no listings.
    max_details == 0 means skip individual detail-page fetches.

    Returns a list of per-subcategory result dicts.
    """
    results = []

    logger.info("─" * 60)
    logger.info("Step 1 — Fetching Animals subcategories")
    subcats = await scraper.get_subcategories()

    if not subcats:
        logger.error("No subcategories returned — check proxy credentials and connectivity")
        return []

    logger.info(f"Found {len(subcats)} subcategories")

    for idx, subcat in enumerate(subcats, 1):
        slug = subcat["slug"]
        name = subcat.get("name_en") or subcat.get("name_ar") or slug

        logger.info("─" * 60)
        logger.info(f"[{idx}/{len(subcats)}] {name}  (slug: {slug})")

        entry = {
            "subcategory": subcat,
            "has_children": False,
            "children": [],
            "listings": [],
            "pages_scraped": 0,
            "detail_fetches": 0,
            "errors": [],
        }

        try:
            # ── Check for child categories ────────────────────────────────────
            children = await scraper.get_catchilds(slug)

            if children:
                entry["has_children"] = True
                entry["children"] = [c.get("slug") for c in children]
                logger.info(f"  {len(children)} child categories found")

                details_fetched = 0
                for child in children:
                    child_slug = child["slug"]
                    child_name = child.get("name_en") or child.get("name_ar") or child_slug
                    logger.info(f"  → Child: {child_name}  ({child_slug})")

                    page = 1
                    while True:
                        if max_pages > 0 and page > max_pages:
                            logger.info(f"    Reached max_pages={max_pages}, stopping")
                            break

                        listings = await scraper.get_listings(
                            slug,
                            page_num=page,
                            child_slug=child_slug,
                            filter_yesterday=False,  # ← scrape EVERYTHING
                        )

                        if not listings:
                            logger.info(f"    Page {page}: empty — stopping pagination")
                            break

                        logger.info(f"    Page {page}: {len(listings)} listings")

                        # Optional detail fetches
                        if max_details > 0:
                            for listing in listings:
                                if details_fetched >= max_details:
                                    break
                                lslug = listing.get("slug")
                                if lslug:
                                    details = await scraper.get_listing_details(
                                        lslug, listing.get("status")
                                    )
                                    if details:
                                        listing["_details"] = details
                                        details_fetched += 1
                                        logger.info(f"      Detail: {lslug}")

                        entry["listings"].extend(listings)
                        entry["pages_scraped"] += 1
                        page += 1
                        await asyncio.sleep(0.5)

                entry["detail_fetches"] = details_fetched

            else:
                # ── No children — scrape main category pages ──────────────────
                logger.info("  No child categories — scraping main pages")
                details_fetched = 0
                page = 1
                while True:
                    if max_pages > 0 and page > max_pages:
                        logger.info(f"  Reached max_pages={max_pages}, stopping")
                        break

                    listings = await scraper.get_listings(
                        slug,
                        page_num=page,
                        filter_yesterday=False,  # ← scrape EVERYTHING
                    )

                    if not listings:
                        logger.info(f"  Page {page}: empty — stopping pagination")
                        break

                    logger.info(f"  Page {page}: {len(listings)} listings")

                    if max_details > 0:
                        for listing in listings:
                            if details_fetched >= max_details:
                                break
                            lslug = listing.get("slug")
                            if lslug:
                                details = await scraper.get_listing_details(
                                    lslug, listing.get("status")
                                )
                                if details:
                                    listing["_details"] = details
                                    details_fetched += 1
                                    logger.info(f"    Detail: {lslug}")

                    entry["listings"].extend(listings)
                    entry["pages_scraped"] += 1
                    page += 1
                    await asyncio.sleep(0.5)

                entry["detail_fetches"] = details_fetched

        except Exception as exc:
            logger.error(f"  Error processing {name}: {exc}")
            entry["errors"].append(str(exc))

        logger.info(
            f"  Subtotal → {len(entry['listings'])} listings, "
            f"{entry['pages_scraped']} pages, "
            f"{entry['detail_fetches']} details"
        )
        results.append(entry)
        await asyncio.sleep(1.0)  # polite gap between subcategories

    return results


# ─── Entry point ─────────────────────────────────────────────────────────────

async def run(args):
    # ── Build proxy credentials ───────────────────────────────────────────────
    # DataImpulse Kuwait geo-targeting: append __cr.kw to username
    user     = os.environ.get("DATAIMPULSE_USER", "2ced8727319e5746c47f__cr.kw")
    password = os.environ.get("DATAIMPULSE_PASS", "ee22dc54c002e494")
    host     = os.environ.get("DATAIMPULSE_HOST", "gw.dataimpulse.com")
    port     = int(os.environ.get("DATAIMPULSE_PORT", "823"))

    session = DataImpulseSession(user=user, password=password, host=host, port=port)

    logger.info("=" * 60)
    logger.info("ANIMALS SCRAPER — DataImpulse Residential Proxy (Kuwait)")
    logger.info("=" * 60)
    logger.info(f"Proxy       : {host}:{port}  (Kuwait residential)")
    logger.info(f"Max pages   : {'unlimited' if args.max_pages == 0 else args.max_pages}  per subcategory")
    logger.info(f"Max details : {'skip' if args.max_details == 0 else args.max_details}  per subcategory")
    logger.info(f"Date filter : NONE — scraping ALL listings regardless of date")
    logger.info(f"Output      : {args.output}")
    logger.info("=" * 60)

    # Late import so our custom session can be injected before any use
    from json_scraper import AnimalsJsonScraper  # noqa: E402

    scraper = AnimalsJsonScraper()
    # Replace the default session with our DataImpulse session
    try:
        scraper.session.close()
    except Exception:
        pass
    scraper.session = session

    start_time = time.time()
    try:
        results = await scrape_all(scraper, args.max_pages, args.max_details)
    finally:
        await scraper.close_browser()

    elapsed = time.time() - start_time

    # ── Compile stats ─────────────────────────────────────────────────────────
    total_listings       = sum(len(r["listings"]) for r in results)
    total_details        = sum(r["detail_fetches"] for r in results)
    total_requests       = session.request_count
    total_cache_hits     = session.cache_hits
    total_fetches_all    = total_requests + total_cache_hits

    output = {
        "test_run":  datetime.now().isoformat(),
        "proxy":     f"{host}:{port} (Kuwait residential — DataImpulse)",
        "config": {
            "max_pages":   args.max_pages,
            "max_details": args.max_details,
            "date_filter": "none — all listings scraped",
        },
        "stats": {
            "elapsed_seconds":          round(elapsed, 1),
            "total_http_requests":      total_requests,
            "cache_hits_free":          total_cache_hits,
            "total_fetches_including_cache": total_fetches_all,
            "total_subcategories":      len(results),
            "total_listings":           total_listings,
            "total_detail_fetches":     total_details,
        },
        "subcategory_summary": [
            {
                "slug":           r["subcategory"]["slug"],
                "name_en":        r["subcategory"].get("name_en"),
                "name_ar":        r["subcategory"].get("name_ar"),
                "has_children":   r["has_children"],
                "children":       r["children"],
                "pages_scraped":  r["pages_scraped"],
                "listings_count": len(r["listings"]),
                "detail_fetches": r["detail_fetches"],
                "errors":         r["errors"],
            }
            for r in results
        ],
        "full_results": results,
    }

    output_path = Path(args.output)
    output_path.write_text(
        json.dumps(output, ensure_ascii=False, indent=2), encoding="utf-8"
    )

    # ── Final report ──────────────────────────────────────────────────────────
    logger.info("")
    logger.info("=" * 60)
    logger.info("FINAL REPORT")
    logger.info("=" * 60)
    logger.info(f"  Elapsed time               : {elapsed:.1f}s")
    logger.info(f"  ┌─ Total HTTP requests made : {total_requests}   (real proxy requests)")
    logger.info(f"  ├─ Cache hits (free)        : {total_cache_hits}   (served from memory)")
    logger.info(f"  └─ All fetches combined     : {total_fetches_all}")
    logger.info(f"  Subcategories scraped       : {len(results)}")
    logger.info(f"  Total listings found        : {total_listings}")
    logger.info(f"  Total detail fetches        : {total_details}")
    logger.info(f"  Output saved to             : {output_path.resolve()}")
    logger.info("")

    if results:
        logger.info("  Per-subcategory breakdown:")
        logger.info(f"  {'Category':<32} {'Listings':>8}  {'Pages':>5}  {'Errors':>6}")
        logger.info("  " + "-" * 56)
        for r in results:
            sc   = r["subcategory"]
            name = sc.get("name_en") or sc.get("name_ar") or sc["slug"]
            err  = len(r["errors"])
            logger.info(
                f"  {name:<32} {len(r['listings']):>8}  {r['pages_scraped']:>5}  "
                + (f"{'⚠ ' + str(err):>6}" if err else f"{'OK':>6}")
            )
    logger.info("")

    if total_listings == 0:
        logger.error("FAIL  No listings found — check proxy credentials and site connectivity")
        sys.exit(1)

    logger.info(f"PASS  Full scrape completed — {total_requests} requests made via DataImpulse proxy")


def main():
    parser = argparse.ArgumentParser(
        description=(
            "Scrape ALL Animals listings on q84sale.com via DataImpulse "
            "residential proxies (Kuwait). No date filter — fetches everything."
        )
    )
    parser.add_argument(
        "--max-pages",
        type=int,
        default=5,
        help="Max listing pages per subcategory (0 = unlimited, default: 5)",
    )
    parser.add_argument(
        "--max-details",
        type=int,
        default=0,
        help="Max detail-page fetches per subcategory (0 = skip, default: 0)",
    )
    parser.add_argument(
        "--output",
        default="animals_dataimpulse_result.json",
        help="Output JSON file path (default: animals_dataimpulse_result.json)",
    )
    args = parser.parse_args()
    asyncio.run(run(args))


if __name__ == "__main__":
    main()
