...
from json_scraper import AnimalsJsonScraper
...
class AnimalsScraperOrchestrator:
    """
    Orchestrates the scraping of animals data with AWS R2 integration
    """
    ...
    async def initialize(self):
        ...
        self.scraper = AnimalsJsonScraper()
            ...
            for result in results:
                try:
                    ...
                    if listings_count > 0:
                        ...
                        # Save a JSON copy for the same dataset under
                        # .../year=YYYY/month=MM/day=DD/json version/{slug}.json
...
async def main():
    ...
    try:
        ...
        logger.info("\n" + "="*60)
        logger.info("ANIMALS SCRAPER STARTING")
        logger.info("="*60)
        logger.info(f"Bucket: {bucket_name}")
        logger.info(f"Max pages per category: {max_pages}")
        
        orchestrator = AnimalsScraperOrchestrator(bucket_name=bucket_name, profile_name=profile_name)
