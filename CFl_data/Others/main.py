...
from json_scraper import OthersJsonScraper
...
class OthersScraperOrchestrator:
    """
    Orchestrates the scraping of 'Others' category data with AWS S3 integration
    Scrapes subcategories: currencies-stamps-and-antiques, books, wholesale, stickers, lost-and-found, other-miscellaneous
    Each subcategory gets its own Excel file with Info summary sheet
    """
    ...
    async def initialize(self):
        ...
        self.scraper = OthersJsonScraper()
    ...
    async def scrape_subcategory(self, subcategory: Dict) -> Dict:
        """
        Scrape an 'Others' subcategory with listings and detailed information
    ...
    async def scrape_all_subcategories(self) -> List[Dict]:
        """
        Scrape all 'Others' subcategories
        Automatically discovers and scrapes all available pages
        """
        try:
            logger.info("Getting 'Others' subcategories...")
            subcategories = self.scraper.get_subcategories()
    ...
    async def save_all_to_s3(self, results: List[Dict]) -> Dict:
        ...
        Creates separate Excel files for each subcategory with Info summary sheet
        """
        upload_summary = {
            "excel_files": [],
            "json_files": [],
        ...
        try:
            ...
            logger.info("Creating Excel files for each subcategory with Info sheet...")
            
            for result in results:
                if result["listings"]:
                    subcategory = result["subcategory"]
                    subcat_slug = subcategory["slug"]
                    subcat_name_ar = subcategory["name_ar"]
                    listings_count = len(result["listings"])
                    
                    logger.info(f"Creating {subcat_slug}.xlsx ({listings_count} listings)...")
                    
                    temp_excel = self.temp_dir / f"{subcat_slug}_temp.xlsx"
                    with pd.ExcelWriter(temp_excel, engine='openpyxl') as writer:
                        # Create Info sheet with summary
                        info_data = [{
                            "Project": "Others - " + subcat_name_ar,
                            "Subcategory Slug": subcat_slug,
                            "Subcategory Name (Arabic)": subcat_name_ar,
                            "Subcategory Name (English)": subcategory["name_en"],
                            "Total Listings": listings_count,
                            "Total Pages Scraped": result.get("total_pages", 0),
                        ...
                        # Create Data sheet with listings
                        df = pd.DataFrame(result["listings"])
                        df.to_excel(writer, sheet_name='Data', index=False)
                        logger.info(f"  Created sheets: Info + Data ({listings_count} listings)")
                    ...
                    if s3_excel_path:
                        ...
                        logger.info(f"✓ Uploaded: {subcat_slug}.xlsx ({listings_count} listings)")
...
async def main():
    """Main entry point for the 'Others' scraper"""
    orchestrator = None
    ...
    try:
        ...
        logger.info("\n" + "="*60)
        logger.info("OTHERS CATEGORY SCRAPER STARTING")
        logger.info("="*60)
        logger.info(f"Bucket: {bucket_name}")
        logger.info(f"Mode: Scrape ALL available pages per subcategory")
        
        orchestrator = OthersScraperOrchestrator(bucket_name=bucket_name, profile_name=profile_name)
        ...
        if results:
            ...
            for excel_file in upload_summary['excel_files']:
                logger.info(f"  - {excel_file['name']}.xlsx ({excel_file['name_ar']}): {excel_file['total_listings']} listings")
