...
from json_scraper import SportJsonScraper
...
class SportScraperOrchestrator:
    """
    Orchestrates the scraping of sport data with AWS R2 integration
    ...
    async def scrape_subcategory(self, subcategory: Dict) -> Dict:
        """
        Scrape a sport subcategory with listings and detailed information
    ...
    async def scrape_all_subcategories(self) -> List[Dict]:
        """
        Scrape all sport subcategories from the main page
        Automatically discovers and scrapes all available pages
        """
        try:
            logger.info("Fetching sport subcategories...")
    ...
    async def save_all_to_R2(self, results: List[Dict]) -> Dict:
        ...
        Creates an Excel file named 'sport' with sheets for each subcategory
        """
        upload_summary = {
            "excel_files": [],
            "json_files": [],
        ...
        try:
            ...
            logger.info("Creating Excel file 'sport' with subcategory sheets...")
            
            temp_excel = self.temp_dir / "sport_temp.xlsx"
            ...
            json_file = self.temp_dir / "sport_temp.json"
            ...
            if R2_json_path:
                ...
                logger.info(f"[OK] Uploaded: sport.json ({total_listings} listings)")
    ...
    async def run(self) -> Dict:
        ...
        try:
            ...
            logger.info("=" * 80)
            logger.info("sport SCRAPER ORCHESTRATOR")
            logger.info("=" * 80)
